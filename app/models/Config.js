export default class Config {
  ...
  async removeDutyColumnByName(name) {
    this.dutyColumns = this.dutyColumns.filter(
      (dutyColumn) => dutyColumn.name !== name
    );
    await this.updateDatabase("dutyColumns", this.dutyColumns);
    this.setState(this);
  }
  ...
  async changeDutyColumnPosition(name, direction) {
    ...
    if (newIndex < 0 || newIndex >= this.dutyColumns.length) {
      console.error(
        "Die Verschiebung ist nicht möglich, da die neue Position außerhalb des Arrays liegt."
      );
      return;
    }
  ...
  async addRule(rule) {
    ...
    await this.updateDatabase("rules", this.rules);
  }

  async removeRuleByIndex(index) {
