...
export default function WishRowMobile({ doctor, index, roster }) {
  const { user, vacations } = usePageContext();
  const [mouseDown, setMouseDown] = useState(false);
  const [showWishDays, setShowWishDays] = useState(false);
  ...
  function dayEntryClass(day, doctor) {
    ...
    const opacityClass =
      day.getDay() === 0 ||
      day.getDay() === 6 ||
      roster.days.find((d) => d.date.getTime() === day.getTime()).holiday
        ? "opacity-40"
        : "";

    return `w-[90%] h-10 rounded-sm mx-1 cursor-pointer flex items-center justify-center ${bgColorClass} ${ringColorClass} ${opacityClass}`;
  }
  ...
  function handleMouseUp(day, doctor) {
    ...
    const isInBlacklist = (checkDay) =>
      doctor.blacklist.some(
        (blacklistDay) => blacklistDay.getTime() === checkDay.getTime()
      );

    // Funktion zum Prüfen, ob ein Tag in der Greenlist enthalten ist
    const isInGreenlist = (checkDay) =>
      doctor.greenlist.some(
        (greenlistDay) => greenlistDay.getTime() === checkDay.getTime()
      );

    if (selectedDaysCount > 1) {
      // Bei Auswahl mehrerer Tage
      const allDaysInBlacklist = selectedDays.every((selectedDay) =>
        isInBlacklist(selectedDay)
      );
